...
void BallisticDemo::fire()
{
	...
	case GRENADE:
		// Note that this is the kind of laser bolt seen in films,
		// not a realistic laser beam!
		shot->particle.setMass(0.45f); // 0.1kg - almost no weight
		shot->particle.setVelocity(0.0f, 50.0f, 30.0f); // 100m/s
		shot->particle.setAcceleration(0.0f, -20.0f, 0.0f); // No gravity
		shot->particle.setDamping(0.99f);
		break;
    }
